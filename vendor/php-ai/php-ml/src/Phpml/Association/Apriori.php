...
class Apriori implements Associator
{
    ...
    private function frequent(array $samples) : array
    {
        return array_filter($samples, function ($entry) {
            if ($this->support($entry) >= $this->support) {
                $this->debug['frequent_support'][] = [
                    'item' => $entry,
    ...
    public function getFrequent(array $samples) : array
    {
        return array_filter($samples, function ($entry) {
            return $this->support($entry) >= $this->support;
        });
    ...
    {
        //echo "confidence: ". implode('-', $set) . '/' . implode('-', $subset).'<br/>';
        //echo "support:".$this->support($set)."-".$this->support($subset).'hasil:' .$this->support($set) / $this->support($subset).'<br/>';
